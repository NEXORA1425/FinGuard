...
import { ShieldCheck, ShieldAlert, ArrowRight, Mail, User as UserIcon, Lock, Sparkles } from 'lucide-react';
...
export const AuthPage: React.FC<AuthPageProps> = ({ onNavigate, redirectAfterLogin = 'home' }) => {
  const { login, loginWithGoogle, signup, error, clearError, loading } = useAuth();
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-xl relative overflow-hidden">
        ...
        <div className="mb-5">
          <button
            ...
            disabled={loading}
            className="w-full py-3 px-4 bg-white hover:bg-slate-50 active:bg-slate-100 border border-slate-300 rounded-xl text-xs sm:text-sm font-bold text-slate-800 shadow-2xs transition-all flex items-center justify-center gap-3 cursor-pointer min-h-[46px]"
          >
        ...
        {/* Separator */}
        <div className="flex items-center gap-3 my-5 text-[11px] font-bold tracking-wider text-slate-400 uppercase">
